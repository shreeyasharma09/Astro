    ...
    <Container>
      ...
      {step === 0 && (
        <PageLayout
          header={
            <>
              <h2 className="text-2xl font-extrabold">Which age range fits you?</h2>
              <p className="text-mute-light dark:text-mute-dark text-sm">This just helps us suggest the right scenarios.</p>
            </>
          }
          actions={<Button disabled={!age} onClick={() => setStep(1)}>Next</Button>}
        >
          {ageOptions.map((a) => (
            <button
              key={a}
      ...
      {step === 1 && (
        <PageLayout
          header={
            <>
              <h2 className="text-2xl font-extrabold">What feels hardest right now?</h2>
              <p className="text-mute-light dark:text-mute-dark text-sm">Pick as many as apply. We'll start your library here.</p>
            </>
          }
          actions={<Button onClick={() => setStep(2)}>{selected.size === 0 ? 'Skip for now' : `Next (${selected.size} selected)`}</Button>}
        >
          {SCENARIOS.map((s) => {
      ...
      {step === 2 && (
        <PageLayout
          header={
            <>
              <h2 className="text-2xl font-extrabold">How should reminders feel?</h2>
              <p className="text-mute-light dark:text-mute-dark text-sm">We'll ping you before and after real situations you schedule.</p>
            </>
          }
          actions={<Button onClick={onDone}>All set — take me in</Button>}
        >
          {remOptions.map((r) => (
            <button
              key={r.id}
